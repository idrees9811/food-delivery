...
import { categories } from '../img/data'
...
const CreateContainer = () => {
  ...
  //   const date = Date.now()
  //   nameId = "Images/"+ Date.now()+imageFile.name
  //   console.log(nameId)
  // }
  // getName()
  ...
  const uploadImage = (e) => {
    ...
    const nameId = "Images/"+ Date.now()+imageFile.name
    // console.log(nameId)
    const storageRef = ref(storage, nameId)
  ...
  const saveDetail = () => {
    ...
    try{
      if((!title || !calories || !imageAsset || !price || !category)){
        setFields(true)
        setMsg("Required Filleds can't be Empty")
        setAlertStatus('danger')
        setTimeout(() => {
          setFields(false)
          setIsLoading(false)
  ...
  // const color = alertStatus === "danger" ? "w-full p-2 rounded-lg text-center text-lg font-semibold bg-red-400 text-red-800": "w-full p-2 rounded-lg text-center text-lg font-semibold bg-emerald-400 text-emerald-800"
  // console.log(color)
  return (
